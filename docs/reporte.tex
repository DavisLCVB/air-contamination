...
contaminación''. Se entrenaron y evaluaron tres estrategias sobre el mismo hold-out
(30\% de test, separado por día calendario para no filtrar horas del mismo día entre
train y test):
...
\midrule
\textbf{RF + class\_weight} (ganador) & 0.828 & \textbf{0.898} & 0.419 & 0.778 & 0.287 \\
XGBoost + scale\_pos\_weight & 0.808 & 0.896 & 0.402 & 0.808 & 0.267 \\
RF + SMOTE (referencia, no desplegado) & \textbf{0.835} & 0.895 & \textbf{0.425} & 0.763 & 0.294 \\
\bottomrule
\end{tabular}%
}
...
la precisión global. \texttt{RF + SMOTE} obtiene el F1 más alto de los tres (0.425 vs.\
0.419), pero el proyecto lo excluye deliberadamente de la comparación de ``mejor
modelo desplegable'' (\texttt{src/\allowbreak models.py}, función \texttt{entrenar\_y\_evaluar\_todo},
línea 564, que descarta expresamente el candidato \texttt{rf\_smote} antes de fijar el
mejor modelo):
...
\texttt{rf\_classweight} (+0.006) no compensa la complejidad añadida de mantener en
producción un pipeline de SMOTE. El modelo ganador y efectivamente servido por el
Panel~2 es, por lo tanto, \textbf{\texttt{rf\_classweight}}, coherente entre
...
\caption{Matriz de confusión del modelo ganador (\texttt{rf\_classweight}).}
\label{fig:confmat}
\end{figure}

\begin{figure}[h!]
\centering
\includegraphics[width=0.75\textwidth]{../models/rf_classweight_shap_summary.png}
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.85\textwidth]{../models/rf_classweight_shap_force.png}
...
\texttt{co=717.6}, el modelo predice una probabilidad de 0.284 para ``alta
contaminación'' -- por debajo del umbral de 0.5, así que clasifica correctamente esta
hora como \emph{baja}, coincidiendo con la etiqueta real. El mayor empuje hacia
``baja'' (azul) viene de \texttt{pm\_10} y, en menor medida, de \texttt{co}: aunque el
PM10 está elevado (82.2), su efecto SHAP en este caso puntual empuja hacia la clase
baja, no hacia la alta -- evidencia de que el modelo no aplica un umbral univariado
simple sobre un solo contaminante, sino que pondera la combinación completa de los 5.
Lo único que empuja hacia ``alta'' (rojo) es el O\textsubscript{3} muy bajo (2.4) y el
NO\textsubscript{2} moderado (40.3), pero no alcanza para revertir el resultado.
...
clasificación de riesgo por hora con F1 = 0.419 y explicación SHAP, útil como insumo de
